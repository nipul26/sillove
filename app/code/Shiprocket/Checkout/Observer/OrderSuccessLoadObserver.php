...
class OrderSuccessLoadObserver implements ObserverInterface
{
    /**
    ...
    public function execute(Observer $observer)
    {
        ...
        if ($this->request->getFullActionName() == 'srcheckout_order_index') {
            $quoteId = $this->shiprocketHelper->getCheckoutSession()->getSrQuoteId();
            if ($quoteId) {
                $quote = $this->shiprocketHelper->getCheckoutSession()->getQuote();
                foreach ($quote->getAllItems() as $item) {
                    $quote->removeItem($item->getItemId());
                }
                $quote->collectTotals()->save();
                $this->cartRepository->save($quote);
                $this->shiprocketHelper->getCheckoutSession()->setCartData(null);
            }
        }
    }
}

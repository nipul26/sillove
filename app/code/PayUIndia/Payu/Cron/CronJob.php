...
{ 
	...
	public function execute()
	{
		try 
		{
			...
			//$searchCriteria->setPageSize(5)->setCurrentPage(1);
			...
			foreach ($orders->getItems() as $order) {
				...
				//$this->logger->info("Order# {$order->getIncrementId()} - Creation Date: {$order->getCreatedAt()} - method ".$methodTitle);
				if($order->canCancel() && 
				...
				//else {
				//	$this->logger->info("Order# {$order->getIncrementId()} - Payment Method: {$methodTitle} - Creation Date: {$order->getCreatedAt()} - Not Canceled");
				//}
			}
